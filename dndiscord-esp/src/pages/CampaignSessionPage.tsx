...
import { CampaignService, mapCampaignResponse, AdvanceSessionRequest, GameSessionResponse } from '@/services/campaign.service';
...
interface MapData { id: string; type: 'map'; title: string; selectedMap?: string; }
type NodeData = SceneData | ChoicesData | CombatData | MapData;

interface TreeConnection { source: { node: string; port: string }; target: { node: string; port: string }; }
interface ParsedTree {
  nodeMap: Map<string, NodeData>;
  edges: Map<string, string>; // "sourceId::port" → targetId
...
function parseTree(json: string): ParsedTree {
  const tree: { nodes: Array<{ type: string; x: number; y: number; data: NodeData }>; connections: TreeConnection[] } = JSON.parse(json);
  ...
  const firstNodeId = edges.get('start-node::start-output') ?? edges.get('start-node::output');
...
const CampaignSessionPage: Component = () => {
  ...
  const followPort = async (port: string, choiceText?: string) => {
    ...
    // Save current node to backend history (fire-and-forget, don't block navigation)
    const sid = sessionId();
    if (sid && !isSaving()) {
      setIsSaving(true);
  ...
  const handleEnd = async () => {
    // Save last node and complete session
    const node = currentNode();
    const sid = sessionId();
    if (sid && node) {
      try {
        ...
        <Show when={!loading() && !error()}>
          <div class="w-full max-w-2xl">
            ...
            <div class="bg-game-dark/60 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
              <Switch fallback={<Show when={!currentNode()}><div class="text-center text-slate-400 py-6">Aucun bloc à afficher.</div></Show>}>
                {/* SCENE */}
                  ...
                  <div class="max-w-2xl mx-auto">
                    ...
                    <div class="flex justify-end">
                      <Show when={hasPort('output')} fallback={<EndBanner />}>
                        <button onClick={() => followPort('output')} class="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold rounded-xl transition-all shadow-lg shadow-purple-500/20">Continuer<ChevronRight class="w-5 h-5" /></button>
                      </Show>
                    </div>
                  </div>
                </Match>
                    ...
                    <Show when={(currentNode() as ChoicesData)?.choices?.length > 0} fallback={<EndBanner />}>
                      <div class="flex flex-col gap-3">
                        <For each={(currentNode() as ChoicesData)?.choices}>
                          {(choice, i) => (
                            <button onClick={() => followPort(`choice-${i()}`, choice)} class="text-left px-5 py-4 bg-emerald-500/10 border border-emerald-500/30 hover:bg-emerald-500/20 hover:border-emerald-500/60 rounded-xl text-white font-medium transition-all flex items-center gap-3 group">
                ...
                <Match when={currentNode()?.type === 'map'}>
                  <div class="max-w-2xl mx-auto">
                    <div class="flex items-center gap-3 mb-6">
                      <div class="w-10 h-10 rounded-xl bg-blue-500/20 border border-blue-500/40 flex items-center justify-center"><MapIcon class="w-5 h-5 text-blue-400" /></div>
                      <div><p class="text-xs text-blue-400 uppercase tracking-wider font-medium">Carte</p><h2 class="text-2xl font-display text-white">{(currentNode() as MapData)?.title || 'Carte sans titre'}</h2></div>
                    </div>
                    <div class="bg-blue-500/5 border border-blue-500/20 rounded-2xl p-6 mb-8 flex flex-col items-center gap-4 text-center">
                      ...
                      <Show when={(currentNode() as MapData)?.selectedMap} fallback={<p class="text-slate-500 italic">Aucune carte sélectionnée pour ce bloc.</p>}>
                        <div><p class="text-sm text-blue-400 mb-1">Carte sélectionnée</p><p class="text-2xl font-display text-white">{(currentNode() as MapData)?.selectedMap}</p></div>
                      </Show>
                    </div>
                    <div class="flex justify-end"><Show when={hasPort('output')} fallback={<EndBanner />}><button onClick={() => followPort('output')} class="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold rounded-xl transition-all shadow-lg shadow-blue-500/20">Continuer<ChevronRight class="w-5 h-5" /></button></Show></div>
                  </div>
                </Match>
              </Switch>
            </div>
