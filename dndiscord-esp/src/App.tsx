import { Component, Show, onMount, createEffect, createSignal } from 'solid-js';
...
const App: Component = () => {
  ...
  onMount(() => {
    console.log('[App] Component mounted, showing mode selection');
  });
  
  // Functions to manage app phase
  const startMode = (mode: GameMode) => {
    console.log('[App] ========== STARTING MODE:', mode, '==========');
    console.log('[App] Setting appPhase to IN_GAME');
    setAppPhase(AppPhase.IN_GAME);
    
    // Wait for engine to be ready before starting game
    const checkEngine = () => {
      if (isEngineReady()) {
        console.log('[App] Engine is ready, starting game in', mode, 'mode');
        ...
        setTimeout(() => {
          console.log('[App] Initializing game now...');
          startGame(mode);
          console.log('[App] Game initialization complete');
        }, 150);
      } else {
        console.log('[App] Engine not ready, checking again...');
  ...
  const returnToMenu = () => {
    console.log('[App] ========== RETURNING TO MENU ==========');
    console.log('[App] Clearing engine state...');
    
    // FIRST: Clear the 3D engine (removes all meshes)
    clearEngineState();
    
    console.log('[App] Clearing units, tiles, and game state...');
    // THEN: Clear all game state stores
    clearUnits();
    clearTiles();
    resetGameState();
    
    console.log('[App] All state cleared, showing mode selection');
  ...
  const restartGame = () => {
    ...
    console.log('[App] ========== RESTARTING GAME IN', currentMode, 'MODE ==========');
    ...
    console.log('[App] Clearing game state stores...');
    clearUnits();
    clearTiles();
    resetGameState();
    ...
    setTimeout(() => {
      console.log('[App] Re-initializing game in', currentMode, 'mode...');
      startGame(currentMode);
      console.log('[App] Game restart complete');
    }, 100);
  };
