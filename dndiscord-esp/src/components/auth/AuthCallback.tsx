...
export default function AuthCallback() {
  ...
  onMount(async () => {
    try {
      ...
      const state = urlParams.get("state"); // URL de retour quand connexion en redirection (popup bloquée)
      ...
      if (window.opener) {
        ...
        // Connexion en redirection (popup bloquée) → retour à l’activité avec le token dans le hash
        const hash = state.includes("#") ? "&" : "#";
        const returnUrl =
          state + hash + "auth_token=" + encodeURIComponent(response.token);
        setStatus("success");
        setTimeout(() => {
